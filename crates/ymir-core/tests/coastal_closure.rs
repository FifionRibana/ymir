//! ADR Finding 75 — the three coastal states, as `.ymir` exports the author can open, with the
//! coast measured at BOTH length scales.
//!
//! Why two scales: `MIN_SPUR_KM` is 1 km = **20.5 cells at 8192²**, so the shipped detector
//! cannot see a 2-to-5-cell indentation — which is exactly the texture the eye reads as "fur".
//! One detector (`coast_shape_thresholds`), two settings, so the columns are commensurable.
//!
//! Three variants, production seed and config, 8192²:
//!   SHIPPED   — the delivered relief;
//!   LAW ON    — `a_c_slope_law = Some(S_ref = 0.1128)`, the TARGET-GRID calibration of
//!               VALIDATION_NOTE §1 as amended, everything else identical. An experiment
//!               export, exactly as Finding 48 did; the production gate stays OFF.
//!   REFERENCE — the coarse field, no FBM and no incision: "what absent looks like".
//!
//! `geo_scale_ratio = 7.5` — the AUTHOR'S practice and what the on-disk manifest carries, not
//! the code's 1.0 default (Finding 68).
//!
//! Run: cargo test -p ymir-core --release --test coastal_closure -- --ignored --nocapture
...
/// VALIDATION_NOTE §1 as amended: the calibration is single-grid at the 8192² target.
/// ⚠️ `stream_power::CHANNEL_HEAD_S_REF` still ships the 2048² value 0.3319; un-gating the law
/// would have to move it. Not this round.
const S_REF_TARGET_GRID: f32 = 0.1128;
...
fn coastal_closure() {
    ...
    for v in [Variant::Shipped, Variant::LawOn, Variant::Reference] {
        let name = label(v);
        eprintln!("\n╔═══════════════ {name} ═══════════════╗");
        let raw = build(v);
