//! ADR 0001 Finding 95 (continuing Finding 90 block C) — Finding 44's item 2 at the SHIPPED `k_time`.
//!
...
//!
//! Run: cargo test -p ymir-core --release --test f95_long_run -- --ignored --nocapture
...
fn f95_long_run() {
    ...
    // ADR Finding 95-B — the long milestones. 300 ≈ 1.8 h and 1 000 ≈ 6.1 h at the measured
    // 22.1 s/pass (Finding 89-D), so each milestone PRINTS as it completes: a partial batch is
    // still a result, and the round asked to launch rather than to wait.
    for &steps in &[300usize, 1000] {
        let t0 = Instant::now();
