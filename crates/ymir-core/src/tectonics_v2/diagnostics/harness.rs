...
//! and runs the coupled thin-sheet + advection loop for the
//! configured number of macro steps. Metrics are aggregated
//! step-by-step.
...
pub struct BaselineConfig {
    ...
    /// Output directory for heightmaps.
...
pub fn run_baseline(cfg: &BaselineConfig) -> BaselineResult {
    ...
    let mut cg_iters: Vec<usize> = Vec::with_capacity(cfg.steps);
    ...
    for step in 0..cfg.steps {
        ...
        cg_iters.push(stats.iterations);
    ...
    let iter_mean = if cg_iters.is_empty() {
        0.0
    } else {
        cg_iters.iter().sum::<usize>() as f64 / cg_iters.len() as f64
    };
    let kappa = condition_number_estimate(iter_mean.round() as usize, cfg.sheet.tol);
    let iter_max = cg_iters.iter().copied().max().unwrap_or(0);
    ...
    let metrics = Metrics {
        ...
        eta_contrast: 1.0,
        cg_iter_mean: iter_mean,
        cg_iter_max: iter_max,
        cg_iter_histogram: IterationHistogram::from_samples(&cg_iters),
    ...
    let config_dump = SolverConfigDump {
        formulation: "thin viscous sheet (elliptic: -∇·(2η ε̇(v)) = f; no pressure, no ∇·v=0 constraint)".into(),
        ...
        preconditioner: "velocity Jacobi (diag(A)⁻¹), null-space wrapped".into(),
        gauge_fixing: "mean(vx), mean(vy) projected before & after every M⁻¹ and once post-solve".into(),
        cg_tol: cfg.sheet.tol,
        cg_max_iter: cfg.sheet.max_iter,
