# Step 6 — Regression (Step 5 physics setup, Step 6 machinery structurally bypassed)

> **Step 6 regression run for milestone "Solver reconstruction".**
> Setup: identical to Step 5 physics — `GpeForce` + yielding Enabled + basal drag Enabled + `BoundaryConfig` with `horizontal_oceanic_strip` layout in **Open mode** (Step 5 rate-based source/sinks). No Voronoi tessellation, no dynamic detection, no recycling buffer. Target: wallclock and CG-iters ratios within `[0.95, 1.05]` of Step 5 physics (`35.058s / 298.899s`, `108.5 / 205.0` CG mean at 64² / 128²).
> The structural bypass is a match-arm dispatch: `RecyclingModeInit::Open` selects the Step 5 `compute_source_sink_terms` path directly; `Closed` would allocate a buffer + accumulators that are never constructed in Open mode.
...
| seed | `42` | ✅ |
| body force | `ForceSum [gpe]: GpeForce (Ar = 0.100 from scales)` | ✅ (GpeForce — same as Step 5 physics) |
| yielding | `Enabled` | ✅ (Enabled, Bi=0.15) |
| basal drag | `Enabled (Br = 0.050, S exponent = 2.0)` | ✅ (Enabled, Br=0.05) |
| boundary | `Enabled [Open (Step 5 rate-based)] (layout='horizontal_oceanic_strip', k_sub=0.500, k_arc=0.150, k_spread=0.050, k_coll-v=0.050, k_rift-v=0.020)` | ✅ (Enabled, Open mode, `horizontal_oceanic_strip`) |
| Voronoi | not built (geometry static) | ✅ |
| dynamic detection | not invoked (geometry_kind == Static) | ✅ |
| recycling buffer | not instantiated (RecyclingModeInit::Open) | ✅ |
...
| boundary config | Enabled [Open (Step 5 rate-based)] (layout='horizontal_oceanic_strip', k_sub=0.500, k_arc=0.150, k_spread=0.050, k_coll-v=0.050, k_rift-v=0.020) |
...
|---|---|---|---|---|
| `docs/reports/step6_regression_heightmaps/s_64x64_t0000.png` | `1.601e-1` | `1.200e0` | `7.375e-1` | `docs/reports/step6_regression_heightmaps/s_64x64_t0000_colorbar.png` |
| `docs/reports/step6_regression_heightmaps/s_64x64_t0150.png` | `1.602e-1` | `1.198e0` | `7.398e-1` | `docs/reports/step6_regression_heightmaps/s_64x64_t0150_colorbar.png` |
| `docs/reports/step6_regression_heightmaps/s_64x64_t0300.png` | `1.604e-1` | `1.197e0` | `7.422e-1` | `docs/reports/step6_regression_heightmaps/s_64x64_t0300_colorbar.png` |

### Numerical regression vs Step 5 physics

Same forcing, same preset, same yielding (Enabled, Bi=0.15), same basal drag (Enabled, Br=0.05), same static `horizontal_oceanic_strip` layout + Step 5 rates as Step 5 physics — with `RecyclingMode::Open`. The Step 6 machinery (Voronoi tessellation, dynamic detection, delayed buffer, immediate accumulators) is structurally bypassed via the Open-mode match arm. Ratio targets: wallclock and CG-iter-per-linear-solve both within `[0.95, 1.05]` vs Step 5 physics (`35.058s / 298.899s`, `108.5 / 205.0` CG mean at 64² / 128²).
...
| max \|mean(vy)\| | 1.754e-20 | 1.754e-20 | bruit machine |
...
| snapshot | min | max | mean | colour-bar |
|---|---|---|---|---|
| `docs/reports/step6_regression_heightmaps/s_128x128_t0000.png` | `1.600e-1` | `1.200e0` | `7.312e-1` | `docs/reports/step6_regression_heightmaps/s_128x128_t0000_colorbar.png` |
| `docs/reports/step6_regression_heightmaps/s_128x128_t0150.png` | `1.602e-1` | `1.199e0` | `7.324e-1` | `docs/reports/step6_regression_heightmaps/s_128x128_t0150_colorbar.png` |
| `docs/reports/step6_regression_heightmaps/s_128x128_t0300.png` | `1.603e-1` | `1.198e0` | `7.336e-1` | `docs/reports/step6_regression_heightmaps/s_128x128_t0300_colorbar.png` |
...
| max \|mean(vy)\| | 2.354e-20 | 2.354e-20 | bruit machine |
