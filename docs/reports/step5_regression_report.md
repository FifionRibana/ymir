# Step 5 — Sinusoidal forcing, boundary disabled (regression mirror of Step 4 physics-yielding-Enabled)

> **Step 5 regression run for milestone "Solver reconstruction".**
> Same setup as Step 4 physics (GPE + basal drag Enabled) **with yielding Enabled (Bi = 0.15)** and with the new flag `BoundaryConfig::Disabled`. The `Disabled` variant **structurally bypasses** the Q evaluation, the clamp, and all tracking; `S̃` evolves through plain advection only.
> Reference: the **Step 5 reference variant** run (`step5_reference_variant_report.md`) — Step 4 physics configuration with yielding Enabled — produced on this branch for regression parity because the merged Step 4 physics ran with yielding Disabled for Br isolation, which no longer matches the new regression convention. Target: wallclock and CG-iters ratios within `[0.95, 1.05]`.
...
## Setup parity with Step 5 reference variant

Contract: a mismatch on any of these disqualifies the comparison as a regression test.

| item | value | same as Step 5 reference variant? |
|---|---|---|
...
| seed | `42` | ✅ |
| body force | `ForceSum [gpe]: GpeForce (Ar = 0.100 from scales)` | ✅ (GpeForce — same as reference variant) |
| yielding | `Enabled` | ✅ (Enabled, Bi=0.15) |
| basal drag | `Enabled (Br = 0.050, S exponent = 2.0)` | ✅ (Enabled, Br=0.05) |
| boundary | `Disabled` | ✅ (Disabled — structural bypass) |
...
| boundary config | Disabled |
| boundary layout | — |
| seed | 42 |

### Timing

- wallclock total: `9.948 s`
- wallclock per step (mean): `33.161 ms`
- steps: `300`

### Linear-solver health (CG inside Newton)

- κ(A) estimate from CG iterations (per Newton step): `2.85e1`
- CG iterations per Newton step — mean: `50.8`, max: `109`
...
- Br = `0.050`
- `basal_drag_energy_ratio` (mean over run of `mean_cells(Br·S̃² / (Br·S̃² + η/Δx²))`): `1.844e-7`
- `drag_vs_visc_diagonal_ratio` (mean over run of `mean_cells(Br·S̃² / (η/Δx²))`): `1.844e-7`
- Algebraic identity check `r/(1+r) ≈ energy_ratio`: predicted `1.844e-7` vs measured `1.844e-7` (relative diff `7.5e-9`; spec bound: coarse, typically `< 1e-1`)
...
- relative drift: `-7.772e-16`

### Null-space health

- max |mean(vx)| across solves: `9.843e-23`
- max |mean(vy)|: `1.746e-22`
...
|---|---|---|---|---|
| `docs/reports/step5_reference_variant_heightmaps/s_64x64_t0000.png` | `8.005e-1` | `1.200e0` | `1.000e0` | `docs/reports/step5_reference_variant_heightmaps/s_64x64_t0000_colorbar.png` |
| `docs/reports/step5_reference_variant_heightmaps/s_64x64_t0150.png` | `8.008e-1` | `1.199e0` | `1.000e0` | `docs/reports/step5_reference_variant_heightmaps/s_64x64_t0150_colorbar.png` |
| `docs/reports/step5_reference_variant_heightmaps/s_64x64_t0300.png` | `8.012e-1` | `1.198e0` | `1.000e0` | `docs/reports/step5_reference_variant_heightmaps/s_64x64_t0300_colorbar.png` |

### Numerical regression vs Step 5 reference variant

Same forcing, same preset, same yielding (Enabled, Bi=0.15), same basal drag (Enabled, Br=0.05) as the Step 5 reference variant, with `BoundaryConfig::Disabled`. The reference variant is documented in `step5_reference_variant_report.md`; it is produced on this branch because the merged Step 4 physics ran with yielding `Disabled` (ad hoc, for Br isolation) and does not match the new Step 5+ regression convention. Ratio targets: wallclock and CG-iter-per-linear-solve both within `[0.95, 1.05]` — the structural by-pass of the boundary pipeline must be zero-cost.

#### Grid 64×64 — comparison vs Step 5 reference variant

| metric | previous | current | ratio / note |
|---|---|---|---|
| wallclock (s) | 9.948 | 9.948 | ×1.00 |
| CG iters / linear solve (mean) | 50.8 | 50.8 | ×1.00 [idéal] |
| S mass drift (relative) | -7.772e-16 | -7.772e-16 | gate 1e-10 |
| max \|mean(vx)\| | 9.843e-23 | 9.843e-23 | bruit machine |
| max \|mean(vy)\| | 1.746e-22 | 1.746e-22 | bruit machine |
...
- wallclock total: `80.120 s`
- wallclock per step (mean): `267.068 ms`
- steps: `300`
...
- Br = `0.050`
- `basal_drag_energy_ratio` (mean over run of `mean_cells(Br·S̃² / (Br·S̃² + η/Δx²))`): `4.610e-8`
- `drag_vs_visc_diagonal_ratio` (mean over run of `mean_cells(Br·S̃² / (η/Δx²))`): `4.610e-8`
- Algebraic identity check `r/(1+r) ≈ energy_ratio`: predicted `4.610e-8` vs measured `4.610e-8` (relative diff `1.9e-9`; spec bound: coarse, typically `< 1e-1`)
- `peak_v_damping_ratio`: — (requires regression run; use `--forcing both` or `--forcing sinusoidal`)
...
- relative drift: `-3.664e-15`

### Null-space health

- max |mean(vx)| across solves: `9.326e-23`
- max |mean(vy)|: `2.927e-22`
...
| snapshot | min | max | mean | colour-bar |
|---|---|---|---|---|
| `docs/reports/step5_reference_variant_heightmaps/s_128x128_t0000.png` | `8.001e-1` | `1.200e0` | `1.000e0` | `docs/reports/step5_reference_variant_heightmaps/s_128x128_t0000_colorbar.png` |
| `docs/reports/step5_reference_variant_heightmaps/s_128x128_t0150.png` | `8.005e-1` | `1.199e0` | `1.000e0` | `docs/reports/step5_reference_variant_heightmaps/s_128x128_t0150_colorbar.png` |
| `docs/reports/step5_reference_variant_heightmaps/s_128x128_t0300.png` | `8.008e-1` | `1.199e0` | `1.000e0` | `docs/reports/step5_reference_variant_heightmaps/s_128x128_t0300_colorbar.png` |

### Numerical regression vs Step 5 reference variant

Same forcing, same preset, same yielding (Enabled, Bi=0.15), same basal drag (Enabled, Br=0.05) as the Step 5 reference variant, with `BoundaryConfig::Disabled`. The reference variant is documented in `step5_reference_variant_report.md`; it is produced on this branch because the merged Step 4 physics ran with yielding `Disabled` (ad hoc, for Br isolation) and does not match the new Step 5+ regression convention. Ratio targets: wallclock and CG-iter-per-linear-solve both within `[0.95, 1.05]` — the structural by-pass of the boundary pipeline must be zero-cost.

#### Grid 128×128 — comparison vs Step 5 reference variant

| metric | previous | current | ratio / note |
|---|---|---|---|
| wallclock (s) | 80.120 | 80.120 | ×1.00 |
| CG iters / linear solve (mean) | 111.4 | 111.4 | ×1.00 [idéal] |
| S mass drift (relative) | -3.664e-15 | -3.664e-15 | gate 1e-10 |
| max \|mean(vx)\| | 9.326e-23 | 9.326e-23 | bruit machine |
| max \|mean(vy)\| | 2.927e-22 | 2.927e-22 | bruit machine |
