# Step 5 — Reference variant (Step 4 physics with yielding Enabled, for Step 5 regression parity)

> **Reference variant run produced on the Step 5 branch for regression parity.**
> Configuration: `GpeForce (Ar = 0.1)` + `YieldingConfig::Enabled (Bi = 0.15)` + `BasalDragConfig::Enabled (Br = 0.05)` + `BoundaryConfig::Disabled`. Differs from the merged Step 4 physics run which had yielding `Disabled` for Br isolation — that ad hoc configuration does not match the new Step 5+ regression convention ("activate all mechanisms through N-1"), so this variant replaces it as the comparison target for the Step 5 regression run.
> This report is structural only: it serves as the wallclock / CG-iters / κ(A) baseline for `step5_regression_report.md`. It is not intended to be cited in the milestone roadmap as a physics milestone in its own right.
...
### Comparison vs Step 4 physics (advisory — yielding re-enabled for Step 5 regression parity, not a regression test)

#### Grid 64×64 — comparison vs Step 4 physics

| metric | previous | current | ratio / note |
|---|---|---|---|
| wallclock (s) | 5.685 | 9.948 | ×1.75 |
| CG iters / linear solve (mean) | 51.5 | 50.8 | ×0.99 [idéal] |
| S mass drift (relative) | -1.110e-15 | -7.772e-16 | gate 1e-10 |
| max \|mean(vx)\| | 6.907e-23 | 9.843e-23 | bruit machine |
| max \|mean(vy)\| | 1.504e-22 | 1.746e-22 | bruit machine |
...
| metric | previous | current | ratio / note |
|---|---|---|---|
| wallclock (s) | 50.436 | 80.120 | ×1.59 |
| CG iters / linear solve (mean) | 117.3 | 111.4 | ×0.95 [idéal] |
| S mass drift (relative) | -5.884e-15 | -3.664e-15 | gate 1e-10 |
| max \|mean(vx)\| | 7.165e-23 | 9.326e-23 | bruit machine |
| max \|mean(vy)\| | 1.487e-22 | 2.927e-22 | bruit machine |
